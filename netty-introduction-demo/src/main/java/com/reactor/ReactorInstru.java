 ...
 *
 * Reactor 模式也叫反应器模式，是大多数IO相关组件如 Netty、Redis在使用的IO模式，为什么需要这种模式，它是如何设计来解决高性能并
 * 发的呢？
 * 最原始的网络编程思路就是服务器用一个 while 循环，不断监听端口是否有新的套接字连接，如果有，那么就调用一个处理函数处理。这种方
 * 法的最大问题是无法并发，效率太低，如果当前的请求没有处理完，那么后面的请求只能被阻塞，服务器的吞吐量太低。之后想到了使用多线程，
 * 也就是很经典的 connection per thread，每一个连接新开一个线程处理，每个线程中都独自处理同样的流程。tomcat服务器的早期版本确实
 * 是这样实现的。
 *
