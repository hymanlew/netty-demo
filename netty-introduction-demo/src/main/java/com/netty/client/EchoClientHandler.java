 ...
 * ChannelPipeline 提供了 ChannelHandler 链的容器。以客户端应用程序为例，如果事件的运动方向是从客户端到服务端的，那么我们称这些事件为出站的，
 * 即客户端发送给服务端的数据会通过 pipeline 中的一系列 ChannelOutboundHandler，并被这些Handler处理，反之则称为入站。
 *
 * ChannelHandlerContext：
 * 它是保存 Channel 相关的所有上下文信息，同时关联一个 ChannelHandler 对象。即 ChannelHandlerContext 中包含一个具体的事件处理器 ChannelHandler，
 * 同时也绑定了对应的 pipeline 和 Channel 的信息，方便对 ChannelHandler 进行调用。在 ChannelPipeline 中按照顺序添加 ChannelHandler，并在
 * 之后按照顺序调用。事实上每个 ChannelHandler 会被先封装成 ChannelHandlerContext。之后再封装进 ChannelPipeline 中。ChannelHandlerContext
 * 的默认实现类是 DefaultChannelHandlerContext。
