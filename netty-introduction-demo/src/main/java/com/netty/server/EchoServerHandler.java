...
public class EchoServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
         ...
         *
         * writeAndFlush 是 write + flush，是将服务器返回给客户端的数据写入到缓存，并刷新。是将数据写到 ChannelPipeline 中当前
         * ChannelHanler 的下一个 ChannelHanler 并进行处理（是出站操作）。
         * 一般需要对这个发送的数据进行编码，并冲刷到远程节点。关闭通道后，操作完成。并且 Netty 会在写的时候自动释放资源。
         */
        ctx.write(in);
    }
