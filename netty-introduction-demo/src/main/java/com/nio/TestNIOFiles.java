...
public class TestNIOFiles {
	...
	public void test6() throws IOException{
		...
		/**
		 * Java的 File 类提供了访问文件的属性，但 File 提供的功能有限。
		 * Java7 的 NIO.2 在 java.nio.file.attribbute 包下提供了大量的工具类，通过这些工具类，开发者可以非常简单地读取，
		 * 修改文件属性，这些工具类主要分为以下两类。
		 *
		 * XxxAttributeView：代表某种文件属性的视图。
		 * XxxAttributes：代表某种文件属性的集合，程序一般通过 XxxAttributeView 对象获取 XxxAttributes。
		 *
		 * BasicFileAttributeView：
		 * 它可以获取或修改文件的基本属性，包括文件的最后修改时间，最后访问时间，创建时间，大小，是否为目录，是否为符号链接等。
		 * 它的 readAttribute() 方法返回一个 BasicFileAttributes 对象，对文件夹基本属性的修改是通过 BasicFileAtributes 对象完成。
		 *
		 * DosFileAttributeView：
		 * 它主要用于获取或修改文件 DOS 相关属性，比如文件是否只读，是否隐藏，是否是系统文件，是否是存档文件等。它的 readAttributes()
		 * 方法返回一个 DosFileAttributes 对象，对这些属性的修改其实是由 DosfileAttributes 对象来完成。
		 */
